...
::: learning-goals
-   Load external data into R using read.csv() or the RStudio Import Dataset interface.
-   Examine the structure of a dataset using str() and determine which variables should be converted to factors.
-   Use the table() function to summarize and visualize categorical variables and their combinations with barplots.
-   Use logical vectors to filter data and compute summaries like means for specific subgroups.
-   Use the aggregate() function to compute summary statistics for quantitative variables grouped by one or more categorical variables.
:::

::: callout-tip
## Supplemental Material

<a href="Reading-Guide/L9-Reading-Guide.pdf" download> 📄 Download the lecture's Reading Guide </a>

<a href="Assignments/Data_200_df.csv" download> 📎 Download the Data used throughout the lecture </a> <br>
<a href="Assignments/Campus_Habits_19.csv" download> 📎 Download the Data used for the lab </a>
:::


## Loading Data into R

Sometimes we are interested in using data that isn't in R. If this is the case, we will have to load the data into R ourselves using a CSV file. To follow along with this lecture writeup, you can download the dataset called "Data_200_df.csv" from the Supplemental Material found above. Now to load data into R there are several different ways we can do it. One way is using the *read.csv()* function where we pass the file path into the function.
...
Converting variables to factors is especially important because many of the tools we use for exploratory data analysis, such as *table()* and *aggregate()*, are designed to work with categorical variables. If our categories are stored as characters or numbers without meaning, our summaries can be misleading or harder to interpret.

::: {.callout-tip title="Try it Out"}
Emmit is working with the `iris` dataset and wants to treat Species as a factor and create a new categorical variable called `SepalLength_group` by cutting Sepal.Length into three groups (Short, Medium, Long). Help show Emmit how he accomplish this task in R.

<details>

<summary>Click to see the solution</summary>

{{< video https://www.youtube.com/embed/-Y2TVJR7y7w?modestbranding=1&rel=0 >}}
...
```{r}
table(Data_200_df$year_f)
barplot(table(Data_200_df$year_f))
```

```{r}
table(Data_200_df$majors_f)
barplot(table(Data_200_df$majors_f))
```

So, this shows us the number of students who are Juniors, and the number of students who are Data majors, but can we determine how many Junior Data majors we have? One way we could do this is by using a logical vector, which is a TRUE/FALSE statement applied to every row of the dataset. This expression evaluates to TRUE only for rows where both conditions are satisfied, and sum() counts how many TRUE values we have.

```{r}
sum(Data_200_df$year_f=="Junior" & Data_200_df$majors_f=="Data")
```
...
```{r}
table(Data_200_df$majors_f, Data_200_df$year_f)
```

::: {.callout-tip title="Try it Out"}
Emmit is now interested in determing how many flowers fall into each `Species` category, each `SepalLength_group` category, and how many fall into the combination of the two groups. Help show Emmit how this can be done in R along with visualizing the data.
...
mean(Data_200_df$ages[Data_200_df$majors_f=="Psych"])
mean(Data_200_df$ages[Data_200_df$majors_f=="Chem"])
mean(Data_200_df$ages[Data_200_df$majors_f=="History"])
```
...
An example of this function can be seen below. The output is a new data frame where each row corresponds to one group and the final column contains the computed summary statistic.

```{r}
aggregate(Data_200_df$ages, by=list(Data_200_df$majors_f), mean)
```
...
aggregate(Data_200_df$ages, 
          by=list(majors = Data_200_df$majors_f, 
                  sports = Data_200_df$sport_f), 
          mean)
```
...
```{r}
aggregate(Data_200_df$ages, by=list(state= Data_200_df$state_f), sd)
```

::: {.callout-tip title="Try it Out"}
Emmit believes that there is a difference in the `Petal.Length` for different `Species` types. Help show Emmit how this can be determined in R using the `aggregate()` function.
...
<div class="chapter-resources-header">
  <a href="https://youtu.be/U7wcOB7548Q" target="_blank">Lecture Video</a>
</div>

<div class="chapter-resources-grid">
  <div><a href="Assignments/Data-200-L9-Inclass-Assignment.pdf" target="_blank">In-Class Exercises</a></div>
  <div><a href="https://youtu.be/rrhstyGNcow" target="_blank">In-Class Exercises Video Solutions</a></div>
</div>

<div class="chapter-resources-footer lab-row">
  <a href="Assignments/Data-200-L9-Lab.pdf" target="_blank">Lab Problems</a>
</div>

<div class="chapter-resources-footer problem-set-row">
  <a href="Assignments/Data-200-L9-Problem-Set.pdf" target="_blank">Problem Set Questions</a>
</div>
