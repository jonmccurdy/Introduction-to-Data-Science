...
\begin{tcolorbox}[enhanced jigsaw, leftrule=.75mm, arc=.35mm, coltitle=black, rightrule=.15mm, bottomrule=.15mm, toprule=.15mm, left=2mm, colbacktitle=quarto-callout-tip-color!10!white, title=\textcolor{quarto-callout-tip-color}{\faLightbulb}\hspace{0.5em}{Supplemental Material}, toptitle=1mm, breakable, colframe=quarto-callout-tip-color-frame, bottomtitle=1mm, titlerule=0mm, colback=white, opacityback=0, opacitybacktitle=0.6]
...
\begin{tcolorbox}[enhanced jigsaw, leftrule=.75mm, arc=.35mm, coltitle=black, rightrule=.15mm, bottomrule=.15mm, toprule=.15mm, left=2mm, colbacktitle=quarto-callout-note-color!10!white, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{Reflection}, toptitle=1mm, breakable, colframe=quarto-callout-note-color-frame, bottomtitle=1mm, titlerule=0mm, colback=white, opacityback=0, opacitybacktitle=0.6]
...
\begin{tcolorbox}[enhanced jigsaw, leftrule=.75mm, arc=.35mm, coltitle=black, rightrule=.15mm, bottomrule=.15mm, toprule=.15mm, left=2mm, colbacktitle=quarto-callout-tip-color!10!white, title=\textcolor{quarto-callout-tip-color}{\faLightbulb}\hspace{0.5em}{Try it Out}, toptitle=1mm, breakable, colframe=quarto-callout-tip-color-frame, bottomtitle=1mm, titlerule=0mm, colback=white, opacityback=0, opacitybacktitle=0.6]
...
\begin{tcolorbox}[enhanced jigsaw, leftrule=.75mm, arc=.35mm, coltitle=black, rightrule=.15mm, bottomrule=.15mm, toprule=.15mm, left=2mm, colbacktitle=quarto-callout-warning-color!10!white, title=\textcolor{quarto-callout-warning-color}{\faExclamationTriangle}\hspace{0.5em}{Warning}, toptitle=1mm, breakable, colframe=quarto-callout-warning-color-frame, bottomtitle=1mm, titlerule=0mm, colback=white, opacityback=0, opacitybacktitle=0.6]
...
\begin{verbatim}
[1] "Female" "Female" "Male"   "Male"   "Female" "Male"   "Male"  
\end{verbatim}
...
\begin{verbatim}
[1] "Medium" "Medium" "Medium" "Small"  "Small"  "Medium" "Large" 
\end{verbatim}
...
\begin{verbatim}
[1] regular regular regular mini    mini    regular huge   
...
\begin{verbatim}
 [1]  5  4  7 10  1  5  8  1  2  9  4  7  3  4  9
\end{verbatim}
...
1   John   21     Math    FALSE
2  Ringo   25     Data     TRUE
3 George   24     Data    FALSE
4   John   21     Math     TRUE
5  Ringo   18 Comp-Sci     TRUE
\end{verbatim}
...
'data.frame':   10 obs. of  4 variables:
 $ names   : chr  "John" "Ringo" "George" "John" ...
 $ ages    : int  21 25 24 21 18 20 22 20 23 19
 $ major   : chr  "Math" "Data" "Data" "Math" ...
 $ commuter: logi  FALSE TRUE FALSE TRUE TRUE TRUE ...
\end{verbatim}
...
\begin{verbatim}
    names ages      major commuter
7    Paul   22 Undeclared     TRUE
8  George   20   Comp-Sci     TRUE
9    John   23       Data    FALSE
10  Ringo   19   Comp-Sci    FALSE
...
\begin{verbatim}
 [1] 21 25 24 21 18 20 22 20 23 19
\end{verbatim}
...
\begin{verbatim}
 [1] "John"   "Ringo"  "George" "John"   "Ringo"  "Paul"   "Paul"   "George"
 [9] "John"   "Ringo" 
\end{verbatim}
...
\begin{verbatim}
 [1] "Math"       "Data"       "Data"       "Math"       "Comp-Sci"  
 [6] "Math"       "Undeclared" "Comp-Sci"   "Data"       "Comp-Sci"  
\end{verbatim}
...
\tightlist
\item
  Explain what a confidence interval represents and how it relates to
  sampling variability.
\item
  Distinguish between confidence intervals constructed using the
  \(z\)-distribution and the \(t\)-distribution.
\item
  Calculate and interpret confidence intervals for population means when
  \(\sigma\) is known and unknown.
\item
  Describe how confidence level, sample size, and standard deviation
  affect the width of a confidence interval.
\end{itemize}
...
\tightlist
\item
  Explain the purpose of hypothesis testing and how it relates to
  confidence intervals.
\item
  Identify the null hypothesis (\(H_0\)) and alternative hypothesis
  (\(H_A\)) in a given context.
\item
  Interpret the meaning of ``rejecting'' and ``failing to reject'' the
  null hypothesis.
\item
  Know how to calculate a test statistic and p-value.
\item
  Connect the idea of sampling variability to the rejection regions in a
  normal distribution.
\end{itemize}
...
\section{Setting Up to Problem}\label{setting-up-to-problem}
...
\section{Possible Conclusions}\label{possible-conclusions}
...
\section{Significance Levels and Test
Statistics}\label{significance-levels-and-test-statistics}
...
\section{Hypothesis Testing Steps}\label{hypothesis-testing-steps}
...
\section{Hypothesis Testing Examples}\label{hypothesis-testing-examples}
...
One of the cool things about data science that you will see in future
courses is the ability to make predictions using a dataset. The most
basic way this can be done is by drawing a line of best fit that is
close to as many data points as possible. We can then use this line to
predict where other values may lie. As you go on to study machine
learning, you will learn even more powerful ways to make predictions and
classifications.

\begin{itemize}
\tightlist
\item
  Explain the purpose of a line of best fit and how it can be used to
  make predictions.
\item
  Identify the independent and dependent variables in a linear
  relationship.
\item
  Interpret the meaning of the slope and intercept in the context of a
  problem.
\item
  Use R to create a line of best fit with \texttt{lm()} and visualize it
  with \texttt{plot()} and \texttt{abline()}.
\item
  Make predictions using a linear model and understand the limitations
  of extrapolation.
\end{itemize}
...
To understand how this will work without going too far into the theory,
we will look at a basic example. Let's say that we are interested in
trying to predict how long (in feet) it will take a car to come to a
complete stop given that they are driving a certain speed. Luckily we
found a dataset in R that contains some of this information and we can
make a visual plot of the data. To do this, we will use the
\texttt{plot()} function and specify the two variables we are wanting to
look at; speed and distance. We will plot the \textbf{independent
variable} on the x-axis (this is the variable we will use to make a
prediction), and the \textbf{dependent variable} on the y-axis (this is
...
\pandocbounded{\includegraphics[keepaspectratio]{Lecture-16-Line-of-Best-Fit_files/figure-pdf/unnamed-chunk-1-1.pdf}}

Looking at the plot above, we can see that the data appears to have a
linear relationship. This means that when we plot the points they appear
to be distributed in roughly a line. Note that they will not always
follow an exact line.

To create the line of best fit we will need to do some math. The whole
idea of it is to be to identify a line which gets as close to all of the
points as possible. Luckily for us we will let R do all of the math, but
it is important to still understand what is happening with the
algorithm. To do this, we will use the \texttt{lm()} (linear model)
function inside of R. We will first pass the \textbf{dependent variable}
(the one we are trying to predict) into the function and then we will
pass the \textbf{independent variable} (the one we are using to make a
prediction) into the function separated by a tilde (\textasciitilde).
...
Using the output above, we are able to build our line-of-best fit. After
all, a line requires two pieces of information; a slope and an
intercept. This is typically written as \(y=mx+b\). The output above
gives us both of those pieces of information. This would allow us to
write the line as:

\[ \text{distance} = 3.932\times \text{speed} - 17.579 \] Given this
line-of-best fit, we can go about interpreting the different
coefficients. Having a slope of \(3.932\) will indicate the distance
will increase on average by \(3.932\) units for each additional \(1\)
mph increase in speed. This should make sense if we plug in a few
``practice'' numbers into the function above (say speeds of 20 and 21)
we can see the expected distance will take \(3.932\) feet longer. A
positive slope would indicate a positive relationship (both of them
increase together), a negative slope would indicate a negative
relationship (as one increases the other decreases), and a slope of 0
would indicate no relationship is present (one variable does not impact
the other variable).

To interpret the y-intercept we would say that when the speed is \(0\)
miles per hour, it will take \(-17.579\) feet for the car to stop. We do
not interpret this literally. Instead, it simply represents where the
line crosses the y-axis and helps position the model mathematically. So,
it is important to note that not every part of the model will have a
meaningful interpretation, especially when the intercept corresponds to
an impossible or unrealistic value.

\section{Making Predictions}\label{making-predictions}

Once we have our line-of-best fit created, we can go about making
predictions and estimations. To visualize the predictions we can plot
both our dataset and the linear model using the \texttt{abline()}
function. This will allow us to see the line-of-best fit on top of our
data, which will help us verify that we put the variables in the correct
order when we built the model.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{linear\_model }\OtherTok{\textless{}{-}} \FunctionTok{lm}\NormalTok{(cars}\SpecialCharTok{$}\NormalTok{dist }\SpecialCharTok{\textasciitilde{}}\NormalTok{ cars}\SpecialCharTok{$}\NormalTok{speed)}

\FunctionTok{plot}\NormalTok{(cars}\SpecialCharTok{$}\NormalTok{speed, cars}\SpecialCharTok{$}\NormalTok{dist)}
\FunctionTok{abline}\NormalTok{(linear\_model, }\AttributeTok{col=}\StringTok{"red"}\NormalTok{, }\AttributeTok{lwd=}\DecValTok{2}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{Lecture-16-Line-of-Best-Fit_files/figure-pdf/unnamed-chunk-3-1.pdf}}

To make predictions we can simply input values into the equation of our
model. So, say we were interested in finding the distance it takes to
stop if we are driving at 20 miles per hour. We can evaluate the
function as:
...
\begin{Shaded}
\begin{Highlighting}[]
\FloatTok{3.932}\SpecialCharTok{*}\DecValTok{20} \SpecialCharTok{{-}} \FloatTok{17.579}
\end{Highlighting}
\end{Shaded}
...
\section{Cautions}\label{cautions}

There are a few specific thing that we need to watch out for when we
make a line-of-best fit. The first being that we need to make sure our
two variables are linearly related. It will not be beneficial to use a
straight line to model two variables if they have a non-linear pattern.
An example of this is the \texttt{data\_210\_census} dataset in the
\texttt{MSMU} library. We can clearly see below that age and salary do
not follow a linear relationship, as the values start increasing and
then around age 50 they start decreasing. Therefore, we would want to
use a different model (one that would be discussed in a machine learning
course) to make predictions on this dataset.

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{library}\NormalTok{(MSMU)}
\FunctionTok{plot}\NormalTok{(data\_210\_census}\SpecialCharTok{$}\NormalTok{age, data\_210\_census}\SpecialCharTok{$}\NormalTok{salary)}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{Lecture-16-Line-of-Best-Fit_files/figure-pdf/unnamed-chunk-5-1.pdf}}

Another thing we should be cautious about is that we can only make
predictions on the range of values that we used to make our model. When
we built our line-of-best fit to model the distance a car will take to
stop given our speed, we only used cars going between 4 and 25 miles per
hour. Therefore, we would not want to make a prediction about a car
going \(60\) miles per hour, because that is too far away from our
training dataset. This is called extrapolation, and it is not reliable
because we do not know whether the relationship continues outside our
data.
